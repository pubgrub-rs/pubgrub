...
impl<V: Version> VersionSet for DiscreteRange<V> {
    type V = V;
    // Constructors
    fn empty() -> Self {
        DiscreteRange::none()
...
pub struct DiscreteRange<V: Version> {
...
impl<V: Version> DiscreteRange<V> {
...
impl<V: Version> DiscreteRange<V> {
    ...
    fn negate_segments(start: V, segments: &[Interval<V>]) -> DiscreteRange<V> {
...
impl<V: Version> fmt::Display for DiscreteRange<V> {
...
pub mod tests {
    ...
    pub fn strategy() -> impl Strategy<Value = DiscreteRange<NumberVersion>> {
    ...
    proptest! {
        ...
        fn intersection_with_any_is_identity(range in strategy()) {
            assert_eq!(DiscreteRange::any().intersection(&range), range);
        }

        #[test]
        fn intersection_with_none_is_none(range in strategy()) {
            assert_eq!(DiscreteRange::none().intersection(&range), DiscreteRange::none());
        }
        ...
        fn intesection_of_complements_is_none(range in strategy()) {
            assert_eq!(range.negate().intersection(&range), DiscreteRange::none());
        }
        ...
        fn union_of_complements_is_any(range in strategy()) {
            assert_eq!(range.negate().union(&range), DiscreteRange::any());
        }
        ...
        fn always_contains_exact(version in version_strat()) {
            assert!(DiscreteRange::exact(version).contains(&version));
        }
        ...
        fn contains_intersection(range in strategy(), version in version_strat()) {
            assert_eq!(range.contains(&version), range.intersection(&DiscreteRange::exact(version)) != DiscreteRange::none());
        }
        ...
        fn from_range_bounds(range in any::<(Bound<u32>, Bound<u32>)>(), version in version_strat()) {
            let rv: DiscreteRange<NumberVersion> = DiscreteRange::from_range_bounds(range);
        ...
        fn from_range_bounds_round_trip(range in any::<(Bound<u32>, Bound<u32>)>()) {
            let rv: DiscreteRange<NumberVersion> = DiscreteRange::from_range_bounds(range);
            let rv2: DiscreteRange<NumberVersion> = rv.bounding_range().map(DiscreteRange::from_range_bounds::<_, NumberVersion>).unwrap_or_else(DiscreteRange::none);
            assert_eq!(rv, rv2);
        }
    }
}
