...
use crate::report_bis::{DefaultStringReporter, DerivationTree, Derived, External};
use crate::term_bis::{self, Term};
    ...
    pub fn relation(&self, terms: impl Fn(&P) -> Option<&'a Term<VS>>) -> Relation<P> {
        ...
        for (package, incompat_term) in self.package_terms.iter() {
            match terms(package).map(|term| incompat_term.relation_with(&term)) {
                Some(term_bis::Relation::Satisfied) => {}
                Some(term_bis::Relation::Contradicted) => {
                    return Relation::Contradicted(package.clone());
                }
                None | Some(term_bis::Relation::Inconclusive) => {
...
pub mod tests {
    ...
    use crate::term_bis::tests::strategy as term_strat;
