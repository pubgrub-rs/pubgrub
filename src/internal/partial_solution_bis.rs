...
impl<P: Package, V: Version> PartialSolution<P, V> {
            ...
            Entry::Occupied(mut occupied) => {
                ...
                match &mut pa.assignments_intersection {
                    ...
                    AssignmentsIntersection::Decision(_) => {
                        panic!("add_derivation called after decision")
                    }
    ...
    /// TODO: also backtrack global_index? or maybe shift to u64 to make sure to not reach the
    /// u32 limit for identifiers for long resolutions? Seems very unlikely ...
    pub fn backtrack(
        &mut self,
        decision_level: DecisionLevel,
        store: &Arena<Incompatibility<P, V>>,
        ...
        self.package_assignments = std::mem::take(&mut self.package_assignments)
            ...
            .filter_map(|(p, mut pa)| {
                if pa.smallest_decision_level > decision_level {
                    ...
                    //
                    // TODO: evaluate if the binary search is worth it here cause that per_package
                    // vec should be rather small compared to previously where we had the whole
                    // history to search.
                    let pos = pa
                        .dated_derivations
    ...
    ///
    /// Question: since I have organized the partial solution in a Map,
    /// I don't have anymore a global history and cannot for sure which assignment is the satisfier
    /// if different package assignments have the same decision level.
    fn find_satisfier<'a>(
        incompat: &Incompatibility<P, V>,
        package_assignments: &Map<P, PackageAssignments<P, V>>,
        store: &Arena<Incompatibility<P, V>>,
    ) -> Map<P, (usize, u32, DecisionLevel)> {
    ...
    fn find_previous_satisfier<'a>(
        incompat: &Incompatibility<P, V>,
        satisfier_package: &P,
        mut satisfied_map: Map<P, (usize, u32, DecisionLevel)>,
        package_assignments: &Map<P, PackageAssignments<P, V>>,
        store: &Arena<Incompatibility<P, V>>,
    ) -> DecisionLevel {
        ...
        let (previous_derivations, mut accum_term) =
            if satisfier_index == satisfier_pa.dated_derivations.len() {
                ...
                let dd = &satisfier_pa.dated_derivations[satisfier_index];
                (
                    &satisfier_pa.dated_derivations[..satisfier_index],
